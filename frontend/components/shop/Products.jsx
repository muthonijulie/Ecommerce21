...
const ProductCard = ({productId,img, brand, name, price ,cartItems, setCartItems}) => {
  ...
  const addToCart = async () => {
    ...
    try {
      ...
      const newItem = { productId, name, price, image: img };
      ...
      <div className={styles.pro}>
        ...
        <div className={styles.des}>
          ...
          <div className={styles.star}>
            {[...Array(4)].map((_, index) => (
              <FaStar key={index<4? "rgb(243, 181, 25)":"#ccc"} size={12} />
            ))}
          </div>
          <h4>{price}/=</h4>
        </div>
      ...
      <section className={styles.sectionP1}>
        <div className={styles.proContainer}>
          {products.map((product) => (
            <ProductCard key={product.id} {...product} cartItems={cartItems} setCartItems={setCartItems} />
          ))}
        </div>
      </section>
